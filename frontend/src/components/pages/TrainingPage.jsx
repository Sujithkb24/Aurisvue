...
const Training = () => {
  ...
  const alphaButtons = Array.from({ length: 26 }, (_, i) => (
    <button
      key={i}
      className={`px-3 py-3 text-sm font-semibold transition rounded-lg ${darkMode
  ...
  const wordButtons = words.wordList.map((word, i) => (
    <button
      key={i}
      className={`px-3 py-2 text-sm font-semibold transition rounded-lg ${darkMode
          ? 'bg-blue-600 hover:bg-blue-500 text-white'
          : 'bg-blue-500 hover:bg-blue-400 text-white'
        }`}
      onClick={() => {
  ...
  return (
    <div className="justify-center align-center">
      <div className={`flex flex-col min-h-screen transition-colors duration-500 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'
        }`}>
        {/* Header */}
        <header className={`sticky top-0 z-10 backdrop-blur-md bg-opacity-90 ${darkMode ? 'bg-gray-900' : 'bg-white'
          } shadow-lg`}>
          ...
          <div className="w-full md:w-3/5 order-2 md:order-1 h-[500px] mt-20">
            <div className={`relative w-full h-full rounded-2xl overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white'
              ...
              <button
                ...
                className={`absolute bottom-4 right-4 p-2 rounded-full ${darkMode
                    ? 'bg-blue-600 hover:bg-blue-500 text-white'
                    : 'bg-blue-500 hover:bg-blue-400 text-white'
            ...
            <div className={`p-6 rounded-xl ${darkMode ? 'bg-gradient-to-r from-blue-900 to-blue-950' : 'bg-white'} shadow-lg`}>
              <div className="flex mb-4 border-b border-gray-700">
                <button
                  ...
                  className={`px-4 py-2 font-medium text-sm ${activeCategory === "alphabets"
                ...
                <button
                  ...
                  className={`px-4 py-2 font-medium text-sm ${activeCategory === "words"
                      ? (darkMode ? 'text-blue-400 border-b-2 border-blue-400' : 'text-blue-600 border-b-2 border-blue-600')
                      : (darkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-800')
                    }`}
                >
